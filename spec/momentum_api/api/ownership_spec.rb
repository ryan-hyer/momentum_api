...
describe MomentumApi::Ownership do
  ...
  let(:user_details_ownership_2020_01_02_CA_R0) { json_fixture("user_details_ownership_2020_01_02_CA_R0.json") }
  ...
  describe '.run' do
    ...
    context 'Card Auto user expiring next week, update off' do
      ...
      let(:mock_man) do
        mock_man = instance_double('man')
        expect(mock_man).to receive(:user_details).exactly(14).times.and_return(user_details_ownership_2020_01_02_CA_R0)
        expect(mock_man).to receive(:user_client).exactly(1).times.and_return mock_user_client
    ...
    context 'Card Auto ownership new, do_live_updates and do_task_update' do
      ...
      let(:mock_man) do
        mock_man = instance_double('man')
        expect(mock_man).to receive(:user_details).exactly(18).times.and_return user_details_ownership_blank
        expect(mock_man).to receive(:user_client).exactly(1).times.and_return mock_user_client
        expect(mock_man).to receive(:print_user_options).exactly(2).times
        mock_man
      end
