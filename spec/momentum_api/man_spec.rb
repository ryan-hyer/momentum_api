...
describe MomentumApi::Man do
  ...
  context 'a non-Owner' do
    ...
    let(:mock_discourse) do
      ...
      expect(mock_discourse).to receive(:schedule).exactly(10).times.and_return(mock_schedule)
      mock_discourse
    end
    ...
    let(:mock_schedule) do
      ...
      expect(mock_schedule).to receive(:user_cases).once
      expect(mock_schedule).to receive(:options).exactly(3).times.and_return(schedule_options)
      mock_schedule
    end
  ...
  context 'print_user_options with updated_option and user_option_print' do
    ...
    describe '.print_user_options' do
      ...
      it 'responds and prints user' do
        expect { man.print_user_options(user_details, fields: user_option_print, nested_user_field: %W(user_fields 5)) }
            .to output(/last_seen_at/).to_stdout
      end
    end
  end
