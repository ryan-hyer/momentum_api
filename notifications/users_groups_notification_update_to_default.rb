...
def member_notifications(client, user)
  ...
  @users_group_users.each do |users_group|
    ...
    if @notification_level != @default_level  # and if @group_name == @target_group_name
      ...
      @users_group_users_after_update.each do |users_group|  # uncomment to check for the update
        if users_group['group_id'] == @user_group_id
          puts "Updated ID:#{@user_group_id} #{@group_name}    Notification Level: #{users_group['notification_level']}    Default: #{@default_level}"
        end
      end
    end
  end
